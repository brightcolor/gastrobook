...
<div id="tab-buchungsregeln" class="settings-panel hidden">
    <form method="POST" action="{{ route('admin.settings.booking-rules') }}" class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-stone-100">
        ...
        <div class="grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
            ...
            <div><label class="mb-1 block text-xs font-semibold text-stone-500">Umbuchungsfrist (Min.) <span class="tip" tabindex="0" data-tip="Bis wie viele Minuten vor dem Termin darf der Gast online umbuchen (Datum/Uhrzeit ändern)? Danach ist der Umbuchen-Link inaktiv.">?</span></label>
                <input type="number" name="modification_deadline_minutes" value="{{ $settings->modification_deadline_minutes }}" class="w-full rounded-lg border-stone-200"></div>
        ...
        <div class="mt-4 border-t border-stone-100 pt-4">
            ...
            <div class="space-y-1.5 text-sm">
                ...
                <label class="flex items-center gap-2"><input type="checkbox" name="require_email_confirmation" value="1" @checked($settings->require_email_confirmation)> E-Mail-Bestätigung verlangen <span class="tip" tabindex="0" data-tip="Jede Online-Buchung wird erst verbindlich, wenn der Gast auf den Bestätigungslink in seiner E-Mail klickt. Schützt vor Fake-Adressen und vor Buchungen auf Vorrat: Wer fünf Termine will, muss fünf Mails öffnen. Kommt der Klick binnen 24 Stunden nicht, wird der Tisch automatisch wieder frei.">?</span></label>
                @if($tenant->isSalon())
