...
@section('content')
@php($du = \App\Models\Location::where('tenant_id', $tenant?->id)->first()?->effectiveSettings()->du() ?? false)
<div class="mx-auto max-w-md rounded-2xl bg-white p-6 shadow-sm">
    ...
    <p class="mt-2 text-center text-sm text-stone-600">
        {{ $du ? 'Gib deine E-Mail-Adresse ein – wir senden dir' : 'Geben Sie Ihre E-Mail-Adresse ein – wir senden Ihnen' }} einen Anmeldelink (kein Passwort nötig).
    </p>
